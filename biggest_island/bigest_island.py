...
def check_islands(position: tuple) -> bool:
    """
...
def check_next_slots(current_row: int, current_slot: int) -> None:
    """
...
def find_biggest_island(islands: list[list]) -> int:
    bigest_island = 0
    for island in islands:
...
while row < rows_count and not end_search:
    while slot < slots_per_row:
        if not temp_island and matrix[row][slot] == 1 and (row, slot) and not check_islands((row, slot)):
            temp_island.add((row, slot))
            slot += 1
            continue
        if matrix[row][slot] == 1:
            if check_islands((row, slot)) or (row, slot) in temp_island:
                slot += 1
                continue
...
print(f"The biggest island has {find_biggest_island(islands)} slots of land!")
